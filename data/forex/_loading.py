...
def load_processed_forex_data(
    ...
    proc_data_path = f'{FOREX_DATA_PATH}/{data_source.value}/Processed'
    data = {}
...
def load_raw_forex_data(
    ...
    if data_source == ForexDataSource.FOREXTESTER:
        return _load_raw_forextester_forex_data(pairs)
    elif data_source == ForexDataSource.HISTDATA:
        return _load_raw_histdata_forex_data(pairs)


def _load_raw_histdata_forex_data(pairs: List[str]) -> Dict[str, pd.DataFrame]:

    histdata_raw_data_path = f'{FOREX_DATA_PATH}/HistData/Raw'
    histdata_data = {}
...
def _load_raw_forextester_forex_data(pairs: List[str]) -> Dict[str, pd.DataFrame]:

    forextester_raw_data_path = f'{FOREX_DATA_PATH}/ForexTester/Raw'
    forextester_data = {}
